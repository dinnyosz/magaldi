...
async def get_feature_graph(
    ...
    # Query elements with feature_memberships to build soft membership sets
    # This captures ALL elements with any membership in each feature (not just primary)
    ...
    # Build mapping of feature_label -> set of element_ids with membership
    feature_label_to_members: dict[str, set[str]] = {}
    for hit in elements_result.get("hits", {}).get("hits", []):
        source = hit["_source"]
        element_id = source["element_id"]
        for membership in source.get("feature_memberships", []):
            label = membership.get("label")
            if label:
                if label not in feature_label_to_members:
                    feature_label_to_members[label] = set()
                feature_label_to_members[label].add(element_id)

    # Process features - first pass to collect feature info
