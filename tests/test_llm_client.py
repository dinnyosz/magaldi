...
class TestResponseFormat:
    """Tests for response_format parameter translation."""

    def test_ollama_json_schema_uses_format_json(self):
        """Ollama should use format='json' when response_format is provided."""
        client = LLMClient(model="ollama_chat/qwen3.5:4b", api_base="http://localhost:11434")

        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = '{"1": {"cv": 9}}'
        ...
        call_kwargs = mock_comp.call_args.kwargs
        # Should use native 'format' param, not 'response_format'
        assert "response_format" not in call_kwargs
        assert call_kwargs["format"] == "json"

    def test_ollama_json_object_format(self):
        """Ollama should convert json_object response_format to format='json'."""
        client = LLMClient(model="ollama/qwen3.5:4b", api_base="http://localhost:11434")
    ...
    def test_ollama_raw_schema_uses_format_json(self):
        """Ollama should use format='json' for any response_format shape."""
        client = LLMClient(model="ollama_chat/qwen3.5:4b", api_base="http://localhost:11434")
    ...
    def test_ollama_structured_output_forces_think_false(self):
        """Ollama structured output should always set think=False."""
        client = LLMClient(model="ollama_chat/qwen3.5:4b", api_base="http://localhost:11434")
        # qwen3.5 is NOT a thinking model, but think=False should still be set
        assert client._is_thinking_model is False
    ...
    def test_ollama_thinking_model_structured_output(self):
        """Thinking model with structured output should set think=False and format='json'."""
        client = LLMClient(model="ollama_chat/qwen3:4b", api_base="http://localhost:11434")
        assert client._is_thinking_model is True
    ...
    def test_non_ollama_response_format_passed_directly(self):
        """Non-Ollama providers should pass response_format as-is."""
        client = LLMClient(model="gpt-4o-mini", api_key="sk-test")
    ...
    def test_no_response_format_no_format_param(self):
        """When response_format is None, neither format nor response_format should appear."""
        client = LLMClient(model="ollama_chat/qwen3.5:4b", api_base="http://localhost:11434")
...
class TestExtractContentEmptyString:
    """Tests for _extract_content handling of empty string content."""

    def test_thinking_model_empty_string_falls_back_to_reasoning(self):
        """Thinking model with content='' should fall back to reasoning_content."""
        client = LLMClient(
        ...
        assert '"cv": 9' in result

    def test_non_thinking_model_empty_string_raises_error(self):
        """Non-thinking model with content='' should raise LLMError."""
        client = LLMClient(model="ollama/llama-3.1:8b", api_base="http://localhost:11434")
    ...
    def test_thinking_model_empty_string_no_reasoning_raises_error(self):
        """Thinking model with content='' and no reasoning_content should raise."""
        client = LLMClient(model="ollama_chat/qwen3:4b", api_base="http://localhost:11434")
